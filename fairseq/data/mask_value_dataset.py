...
class MaskValueDataset(BaseWrapperDataset):
    """
    A wrapper Dataset for masked language modeling for trace values.
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            # make sure this is value sequence, which must have ## (the dummy token) appeared
            ...
            num_mask = int(
                ...
                self.mask_prob * len(real_bytes) / float(self.mask_multiple_length)
                + np.random.rand()
            )
