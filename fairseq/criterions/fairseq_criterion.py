...
class FairseqCriterion(_Loss):
    def __init__(self, task):
        ...
        if hasattr(task, "target_dictionary"):
            # support multifield target, e.g., what if we have multiple targets?
            if isinstance(task.target_dictionary, dict):
                tgt_dict = task.target_dictionary
                self.padding_idx_dict = {
                    field: tgt_dict[field].pad() if task.target_dictionary[field] is not None else -100
                    for field in task.target_dictionary}
            else:
                tgt_dict = task.target_dictionary
