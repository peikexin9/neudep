...
rm checkpoints/pretrain_layer5 -rf

mkdir -p checkpoints/pretrain_layer5
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python train.py \
  ...
  --no-epoch-checkpoints --save-dir checkpoints/pretrain_layer5/ \
  --encoder-layers $ENCODER_LAYERS \
  --random-token-prob 0.5 --mask-prob 0.2 \
  --memory-efficient-fp16 --batch-size-valid 32 \
  --restore-file checkpoints/pretrain_layer5/checkpoint_best.pt \
  --input-combine fuse --fuse-layer 5 --beta-shift 1e-1 --fuse-dropout 1e-1 --fp16-scale-tolerance 0.25 \
  --byte-combine cnn --code-value-loss-alpha 1e2 |
  tee result/pretrain_layer5
