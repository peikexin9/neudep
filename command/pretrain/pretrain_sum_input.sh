...
rm checkpoints/pretrain_sum_input -rf

mkdir -p checkpoints/pretrain_sum_input
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python train.py \
  ...
  --no-epoch-checkpoints --save-dir checkpoints/pretrain_sum_input/ \
  --encoder-layers $ENCODER_LAYERS \
  --random-token-prob 0.5 --mask-prob 0.2 \
  --memory-efficient-fp16 --batch-size-valid 32 \
  --restore-file checkpoints/pretrain_sum_input/checkpoint_best.pt \
  --input-combine sum --fp16-scale-tolerance 0.25 \
  --byte-combine cnn --code-value-loss-alpha 1e2 |
  tee result/pretrain_sum_input
