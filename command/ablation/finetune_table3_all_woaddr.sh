...
CHECKPOINT_PATH=checkpoints/finetune_table3_all_woaddr
rm -rf $CHECKPOINT_PATH
mkdir -p $CHECKPOINT_PATH
cp checkpoints/pretrain_woaddr/checkpoint_best.pt $CHECKPOINT_PATH/
...
CUDA_VISIBLE_DEVICES=1 python train.py \
  ...
  --no-addr |
  tee result/finetune_table3_all_woaddr
