...
CHECKPOINT_PATH=checkpoints/finetune_table3_all_fusing_3
rm -rf $CHECKPOINT_PATH
mkdir -p $CHECKPOINT_PATH
cp checkpoints/pretrain_fusing_3/checkpoint_best.pt $CHECKPOINT_PATH/
...
CUDA_VISIBLE_DEVICES=3 python train.py \
  data-bin/finetune_table3_all \
  --num-classes $NUM_CLASSES \
  --task mem_dep --criterion mem_dep \
  ...
  --input-combine fuse --fuse-layer 3 --beta-shift 1e-1 --fuse-dropout 1e-1 \
  --byte-combine cnn --num-workers 4 --skip-invalid-size-inputs-valid-test |
  tee result/finetune_table3_all_fusing_3
