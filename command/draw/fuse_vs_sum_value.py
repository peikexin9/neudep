...
with open('result/pretrain', 'r') as f:
    for line in f:
        if '\"value_loss_mse\"' in line:
            line_json = json.loads(line.split('|')[-1])
            fuse.append(float(line_json['value_loss_mse']))

with open('result/pretrain_sum', 'r') as f:
    for line in f:
        if '\"value_loss_mse\"' in line:
            line_json = json.loads(line.split('|')[-1])
            sum.append(float(line_json['value_loss_mse']))

with open('result/pretrain_old', 'r') as f:
    for line in f:
        if '\"value_loss_mse\"' in line:
            line_json = json.loads(line.split('|')[-1])
            fuse_old.append(float(line_json['value_loss_mse']))

with open('result/pretrain_sum_old', 'r') as f:
    for line in f:
        if '\"value_loss_mse\"' in line:
            line_json = json.loads(line.split('|')[-1])
            sum_old.append(float(line_json['value_loss_mse']))
...
# plt.show()
plt.savefig('figs/fuse_vs_sum_value.png', transparent=True, bbox_inches='tight', pad_inches=0, dpi=200)
